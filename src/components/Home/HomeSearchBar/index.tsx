...
export const HomeSearchBar = forwardRef<HomeSearchBarHandle, HomeSearchBarProps>(
  function HomeSearchBar({ className }, ref) {
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
      ...
      if (e.key === "ArrowDown") {
        ...
        e.preventDefault();
        setActiveIndex((i) =>
          i < 0 ? 0 : Math.min(i + 1, allItems.length - 1),
        );
      } else if (e.key === "ArrowUp") {
        if (!allItems.length) return;
        e.preventDefault();
        setActiveIndex((i) =>
          i < 0 ? allItems.length - 1 : Math.max(i - 1, 0),
        );
      } else if (e.key === "Enter") {
        e.preventDefault();
